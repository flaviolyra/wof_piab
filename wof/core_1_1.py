...
class WOF_1_1(object):
    ...
    organization = 'FL_Hydro'
    address = 'Rua General Glicerio 107 ap 301 - Laranjeiras'
    city = 'Rio de Janeiro'
    state = 'RJ - Brazil'
    zipcode = '22245-120'
    contactname = 'Flavio Lyra'
    contactemail = 'flavio.jose.lyra@gmail.com'
    phone = ''
    link = 'https://github.com/flaviolyra'
    ...
    def get_site_code(self, siteArg):
        # Flavio:
        # colon splits siteArg in two parts (networkname and siteCode)
        # if no colon, siteCode is siteArg
        # if networkname is not present network, returns None
        # otherwise returns siteCode
        if ':' in siteArg:
    ...
    def get_variable_code(self, varArg):
        ...
        if ':' in varArg:
        # Flavio:
        # colon splits varArg in two parts (vocabulary and varCode)
        # if no colon, varCode is varArg
        # if vocabulary is not current vocabulary, returns None
        # otherwise returns varCode
    ...
    def create_get_variable_info_response(self, varArg=None):
        ...
        for v in variableResultArr:
          logging.debug('VariableID: {!s}, VariableCode: {!s}, '.format(v.VariableID,v.VariableCode)+
                        'VariableName: {!s}, VariableUnitsID: {!s}, '.format(v.VariableName,v.VariableUnitsID)+
                        'SampleMedium: {!s}, ValueType: {!s}, '.format(v.SampleMedium,v.ValueType)+
                        'IsRegular: {!s}, TimeSupport: {!s}, '.format(v.IsRegular,v.TimeSupport)+
                        'TimeUnitsID: {!s}, DataType: {!s}, '.format(v.TimeUnitsID,v.DataType)+
                        'GeneralCategory: {!s}, NoDataValue: {!s}'.format(v.GeneralCategory,v.NoDataValue))
        for variableResult in variableResultArr:
            v = self.create_variable_element(variableResult)
            logging.debug('create_get_variable_info_response: variable {!s} created'.format(variableResult.VariableCode))
    ...
    def create_get_values_response(self, siteArg, varArg, startDateTime=None,
        ...
        logging.debug('create_get_values_response: timeSeriesResponse created with queryInfo')
        ...
        if isinstance(valueResultArr, dict):
            ...
            logging.debug('create_get_values_response: valueResultArr is not dict')
            timeSeries = self.create_timeseries(
                siteCode,
                varCode,
                valueResultArr
    ...
    def create_timeseries(self, siteCode, varCode, valueResultArr):
        ...
        siteResult = self.dao.get_site_by_code(siteCode)
        logging.debug('create_timeseries: siteResult generated')
        ...
        logging.debug('create_timeseries: varResult generated')
        ...
        # Flavio:
        # Create values, a TsValuesSingleVariableType element:
        values = WaterML.TsValuesSingleVariableType()
        logging.debug('create_timeseries: values created')
        ...
        if methodIdSet:
            ...
            logging.debug('create_timeseries: method_elements created in values')
        ...
        if sourceIdSet:
            ...
            logging.debug('create_timeseries: source_elements created in values')
        ...
        if qualifierIdSet:
            ...
            logging.debug('create_timeseries: qualifier_elements created in values')
       ...
       # Add offset elements
        if offsetTypeIdSet:
            ...
            logging.debug('create_timeseries: offset_elements created in values')
        ...
        if qualitycontrollevelIdSet:
            ...
            try:
                ...
                for qlevelID in qlevelIdIdArr:
                    ...
                    values.add_qualityControlLevel(qlevel)
            logging.debug('create_timeseries: qualityControl_elements created in values')
        ...
        logging.debug('create_timeseries: values added to timeseries')
    ...
    def create_get_values_site_response(self, site,
        ...
        timeSeriesResponse = WaterML.TimeSeriesResponseType()
        logging.debug('create_get_values_site_response: timeSeriesResponse created')
        queryInfo = WaterML.QueryInfoType(creationTime=datetime.datetime.now())
        ...
        # queryInfo.set_extension('')
        timeSeriesResponse.set_queryInfo(queryInfo)
        logging.debug('create_get_values_site_response: queryInfo set to timeSeriesResponse')

        siteCode = self.get_site_code(site)
        seriesResultArr = self.dao.get_series_by_sitecode(siteCode)
        logging.debug('create_get_values_site_response: seriesResultArr: {} series'.format(len(seriesResultArr)))
        if seriesResultArr:
            for seriesResult in seriesResultArr:
                valueResultArr = self.dao.get_datavalues_series(seriesResult, startDateTime, endDateTime)
                logging.debug('create_get_values_site_response: valueResultArr determined')
                # if not valueResultArr:
                #    raise Exception(
