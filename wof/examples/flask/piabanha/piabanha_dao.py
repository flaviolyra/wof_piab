...
import wof.examples.flask.piabanha.piabanha_models as model
...
class OdmDao(BaseDao):
    ...
    def get_sites_by_codes(self, site_codes_arr):
        # Flavio:
        # returns an array of Site objects corresponding to the array
        # of siteCodes
        return model.Site.query.filter(
    ...
    def get_datavalues_series(self, series, begin_date_time=None, end_date_time=None):

        site_id = series.SiteID
        variable_id = series.VariableID
        method_id = series.MethodID
        source_id = series.SourceID
        qual_ctl_id = series.QualityControlLevelID
        logging.debug('get_datavalues_series: site_id: {}, variable_id: {}'.format(site_id, variable_id)
                                          + ' method_id: {}, source_id: {}'.format(method_id, source_id)
                                          + ' qual_ctl_id: {}'.format(qual_ctl_id))
        valueResultArr = None
        ...
        logging.debug('get_datavalues_series: begin_datetime: {}, end_datetime: {}, using_utc: {}'.format(begin_datetime,
                                                                    end_datetime, using_utc))

        if using_utc:
            if begin_datetime and end_datetime:
                valueResultArr = model.DataValue.query.filter(
                    and_(model.DataValue.SiteID == site_id,
                         model.DataValue.VariableID == variable_id,
                         model.DataValue.MethodID == method_id,
                         model.DataValue.SourceID == source_id,
                         model.DataValue.QualityControlLevelID == qual_ctl_id,
        ...
        # Format dates as ISO date strings
        if valueResultArr:
            logging.debug('get_datavalues_series: valueResultArr generated: {!s} values'.format(len(valueResultArr)))
            create_iso_offset = self.create_iso_utc_offset
            ...
            logging.debug('get_datavalues_series: valueResultArr is None')
    ...
    def get_datavalues(self, site_code, var_code, begin_date_time=None,
        ...
        logging.debug('get_datavalues: siteResult generated')
        varResult = self.get_variable_by_code(var_code)
        logging.debug('get_datavalues: varResult generated')
        ...
        if siteResult and varResult:
            ...
            end_datetime = parse_result[1]
            using_utc = parse_result[2]
            logging.debug('get_datavalues: begin_datetime: {}, end_datetime: {}, using_utc: {}'.format(begin_datetime, end_datetime,
                          using_utc))
        ...
        logging.debug('get_datavalues: valueResultArr generated: {!s} values'.format(len(valueResultArr)))
        # Format dates as ISO date strings
        if valueResultArr:
            logging.debug('get_datavalues: valueResultArr generated: {!s} values'.format(len(valueResultArr)))
            create_iso_offset = self.create_iso_utc_offset
            ...
            logging.debug('get_datavalues: valueResultArr is None')
        return valueResultArr
